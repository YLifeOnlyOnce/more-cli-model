...
2. 依次询问项目名、输出目录、模板、是否生成 README / `.gitignore`
3. 如果目标目录已存在且非空，再额外确认一次
4. 从模板注册表里找到真实模板目录
5. 把模板目录交给 `services/scaffold.ts` 递归复制并替换少量变量

这和 Vite、Create React App 一类脚手架的核心体验已经非常接近了，只是这里保留了最小实现，方便你看清楚交互、模板注册表和目录复制是怎么接上的。
